"""Initiate TFX pipeline components for Adult Income Dataset"""
...
def init_components(
    ...
    Args:
        ...
        training_module (str): Path to trainer module
        tuner_module (str): Path to tuner module
        training_steps (int): Number of training steps
        eval_steps (int): Number of eval steps
        serving_model_dir (str): Path to save serving model

    Returns:
        tuple: TFX components
    """

    # 1. ExampleGen with train/eval split
    ...
    # 8. Resolver (ambil model terbaik sebelumnya)
    ...
    # 9. Evaluator
    slicing_specs = [
        tfma.SlicingSpec(),  # global
        tfma.SlicingSpec(feature_keys=["sex"]),  # fairness by gender
        tfma.SlicingSpec(feature_keys=["race"]),  # fairness by race
    ]
    metrics_specs = [
        tfma.MetricsSpec(
            metrics=[
